...
def analyze(data, rules):
    ...
    breakeven = int(round(cap_income / R)) if R else None
    binding = min([("R% x investment", term_R), ("3% x total income", term_income),
                   ("Tk 10,00,000", term_abs)], key=lambda t: t[1])[0]
...
def render_en(a):
    L = [f"# Tax Optimization Report — AY {a['assessmentYear']}",
         ...
         f"- (c) Tk 10,00,000 = **{fmt(a['term_abs'])}**",
    ...
    for e in a["sch5"]:
        cap = fmt(e["cap"]) if e["cap"] is not None else "verify/yearly"
        status = "not checked — fill cap in tax-rules" if e["cap"] is None else ("unused room" if e["cap"] > e["claimed"] else "at/over cap — verify")
...
def render_bn(a, en):
    L = ["", "---", "", "## 🇧🇩 বাংলা", "",
         ...
         f"- (a) R% × বিনিয়োগ = {fmt(a['term_R'])} · (b) আয়ের 3% = {fmt(a['term_income'])} · (c) ১০ লাখ = {fmt(a['term_abs'])}",
    ...
    for e in a["sch5"]:
        cap = fmt(e["cap"]) if e["cap"] is not None else "যাচাই প্রয়োজন"
        status = "ক্যাপ নেই — rules ফাইলে যোগ করুন" if e["cap"] is None else ("জায়গা আছে" if e["cap"] > e["claimed"] else "সীমায় — যাচাই")
...
def main():
    ap = argparse.ArgumentParser(description="Tax optimization report from baseline + deltas (offline, no API).")
